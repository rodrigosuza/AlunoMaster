            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 md:gap-3 flex-1">
                  <span className={`w-6 h-6 md:w-7 md:h-7 rounded-lg flex items-center justify-center font-black text-[10px] md:text-xs shrink-0 ${isSelected ? 'bg-indigo-600 text-white' : 'bg-white text-slate-400 border border-slate-100'
                    }`}>
                    {String.fromCharCode(65 + idx)}
                  </span>
                  <span className="text-xs md:text-sm leading-snug break-words flex-1">{option}</span>
                </div>
                {isAnswered && isCorrect && <CheckCircle2 className="text-emerald-600 shrink-0 ml-2" size={16} />}
                {isAnswered && isSelected && !isCorrect && <XCircle className="text-red-600 shrink-0 ml-2" size={16} />}
              </button>
            );
          })}

          {/* Explanation shows within the scrollable area to prevent master layout scrolling - HIDDEN ON MOBILE */}
          {showExplanation && (
